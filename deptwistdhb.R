...
### colnames(dd)
#subnational.R run here and set as datareduce
datareduce <- domcomplete
#datareduce = data[,c(13,14,20,15:17,66,9,31,68:70,32,34)]

### Getting the year our of date of death.
dmy = as.Date(datareduce[,2],"%d/%m/%Y")
datareduce[,2] = dmy
...
mdf = pp[which(pp$ETHNICG1 == 21|#pp$ETHNICG1 == 32|
         ...
         c(2,3,4,8,12,14,17,18,19,20)]
nmdf = pp[which(!(pp$ETHNICG1 == 21|#pp$ETHNICG1 == 32|
                    pp$ETHNICG2 == 21|#pp$ETHNICG2 == 32|
                    pp$ETHNICG3 == 21)),
          c(2,3,4,8,12,14,17,18,19,20)]#c(1,15,2,16,14,12)]
...
adf = pp[,c(2,3,4,8,12,14,17,18,19,20)]#c(1,15,2,16,14,12)]

daaggt = aggregate(DOD~GENDER+year+ageband+tlatext,data = adf, FUN = function(x){NROW(x)})
daaggd = aggregate(DOD~GENDER+year+ageband+dhbname,data = adf, FUN = function(x){NROW(x)})
...
dmaggt = aggregate(DOD~GENDER+year+ageband + tlatext,data = mdf, FUN = function(x){NROW(x)})
dmaggd = aggregate(DOD~GENDER+year+ageband + dhbname,data = mdf, FUN = function(x){NROW(x)})
...
dnmaggt = aggregate(DOD~GENDER+year+ageband + tlatext,data = nmdf, FUN = function(x){NROW(x)})
dnmaggd = aggregate(DOD~GENDER+year+ageband + dhbname,data = nmdf, FUN = function(x){NROW(x)})
...
csvcreator = function(type,n,givenagg,dep){
  ...
  #}else{
    colnames(agg) = c("G","Year","ageband","dhb","deaths")
  #}
  ...
  newdf$dhb <- rep(dhbtext, each = 24)
  
  #fill empty frame
  
  for(i in 1:length(agg$Year)){
  #  if(dep == 0){
      newdf[which(newdf$Year == agg$Year[i]  & newdf$Factor == agg$Factor[i] & newdf$dhb == agg$dhb[i]),which(agg$ageband[i] == colnames(newdf))] = agg$deaths[i]
  }
  #  }else if(dep>10){
    #   if(dep>100){
    #     d1 = 9
  ...
  write.csv(newdf,file=paste("mortnonmaoridhb.csv",sep=""),row.names=FALSE)
}
