...
dmy = as.Date(datareduce[,3],"%d/%m/%Y")
datareduce[,3] = dmy
...
mdf = pp[which(pp$ETHNICG1 == 21|#pp$ETHNICG1 == 32|
         ...
         c(3,17,4,18,13,15,1,16)]
nmdf = pp[which(!(pp$ETHNICG1 == 21|#pp$ETHNICG1 == 32|
                    pp$ETHNICG2 == 21|#pp$ETHNICG2 == 32|
                    pp$ETHNICG3 == 21)),
          c(3,17,4,18,13,15,1,16)]#c(1,15,2,16,14,12)]
...
adf = pp[,c(3,17,4,18,13,15,1,16)]#c(1,15,2,16,14,12)]

daagg = aggregate(DOD~GENDER+year+ageband+tlatext,data = adf, FUN = function(x){NROW(x)})
...
dmagg = aggregate(DOD~GENDER+year+ageband + tlatext,data = mdf, FUN = function(x){NROW(x)})
...
dnmagg = aggregate(DOD~GENDER+year+ageband + tlatext,data = nmdf, FUN = function(x){NROW(x)})
...
csvcreator = function(type,n,givenagg,dep){
  ...
  if(dep == 0){
    ...
    colnames(agg) = c("G","Year","ageband","tla","deaths")
  }
  ...
  newdf$tla <- rep(tlatext, each = 24)
  
  #fill empty frame
  
  for(i in 1:length(agg$Year)){
  #  if(dep == 0){
      newdf[which(newdf$Year == agg$Year[i]  & newdf$Factor == agg$Factor[i] & newdf$tla == agg$tla[i]),which(agg$ageband[i] == colnames(newdf))] = agg$deaths[i]
  }
  #  }else if(dep>10){
      if(dep>100){
        d1 = 9
  ...
  write.csv(newdf,file=paste("morttotaltla.csv",sep=""),row.names=FALSE)
}
